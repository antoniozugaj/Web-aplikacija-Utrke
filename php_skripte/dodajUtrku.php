<?php
    ...
    if(isset($_POST["naziv"])){
        ...
        $tip_id = (int)$_POST["tip"];
        $naziv= $_POST["naziv"];
        $sudionici=(int)$_POST["sudionici"];

        $prijava="";
        $string=str_replace(".",":",$_POST["prijava"]);
		$string=str_replace(" ","",$string);
		$podjela = explode(":",$string);
		if(count($podjela) == 6){
			$prijava=date("Y-m-d H:i:s", mktime($podjela[3],$podjela[4],$podjela[5],$podjela[1],$podjela[0],$podjela[2]));
		} 
        ...
        $sql = "INSERT INTO utrka (naziv, vrijeme_zavrsetka_prijava, zakljucana, pocetak_odrzavanja, kraj_odrzavanja, broj_sudionika, broj_prijava, drzava_id, tip_utrke_id) VALUE ('$naziv', '$prijava', 0, '$pocetak', '$kraj', $sudionici, 0, $drzava_id, $tip_id)";
        ...
        if($rezultat->num_rows > 0){
            ...
            $sql = "INSERT INTO log (vrijeme, dogadaj, korisnik_id) VALUE ( NOW(), '{$_SESSION["username"]} KREIRANJE UTRKE: $naziv', $user_id)";
            $baza->selectDB($sql);
        }
